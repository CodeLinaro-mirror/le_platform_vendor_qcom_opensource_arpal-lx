...
SpeakerProtectionwsa884x::~SpeakerProtectionwsa884x()
{
    viTxSetupThrdCreated = false;
}

int SpeakerProtectionwsa884x::spkrStartCalibration()
{
    FILE *fp;
...
int SpeakerProtectionwsa884x::viTxSetupThreadLoop()
{
 ...
 */
int32_t SpeakerProtectionwsa884x::spkrProtProcessingMode(bool flag)
{
    int ret = 0, dir = TX_HOSTLESS, flags, viParamId = 0;
    char mSndDeviceName_vi[128] = {0};
    ...
    if (flag) {
        ...
        if(!viTxSetupThrdCreated) {
            viTxSetupThread = std::thread(&SpeakerProtectionwsa884x::viTxSetupThreadLoop,
                    this);
