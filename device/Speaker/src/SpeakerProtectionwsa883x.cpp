...
std::thread SpeakerProtectionwsa883x::viTxSetupThread;
...
SpeakerProtectionwsa883x::~SpeakerProtectionwsa883x()
{
    viTxSetupThrdCreated = false;
}

int SpeakerProtectionwsa883x::spkrStartCalibration()
{
    FILE *fp;
...
int SpeakerProtectionwsa883x::viTxSetupThreadLoop()
{
    ...
    if (spkrProtPayload.operationMode) {
        ...
        switch (spkrProtPayload.operationMode) {
            ...
            break;
            default:
                PAL_ERR(LOG_TAG, "Invalid SP Processing mode!!");
        }
    }
 ...
 */
int32_t SpeakerProtectionwsa883x::spkrProtProcessingMode(bool flag)
{
    int ret = 0, dir = TX_HOSTLESS, flags, viParamId = 0;
    char mSndDeviceName_vi[128] = {0};
    ...
    if (flag) {
        ...
        if(!viTxSetupThrdCreated) {
            viTxSetupThread = std::thread(&SpeakerProtectionwsa883x::viTxSetupThreadLoop,
                    this);
...
void SpeakerProtectionwsa883x::updateCpsCustomPayload(int miid)
{
    PayloadBuilder* builder = new PayloadBuilder();
    uint8_t* payload = NULL;
    size_t payloadSize = 0;
